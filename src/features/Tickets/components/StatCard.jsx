    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <p
            className={`text-3xl font-black text-gray-900 mt-2 tracking-tight ${
              isActive ? activeStyles.value : ""
            }`}
          >
        ...
        <div
          className={`
            ...
            ${isActive ? activeStyles.iconBg : ""}
          `}
        >
          {Icon ? (
            <Icon
              size={14}
              className={isActive ? activeStyles.iconText : "text-primary-600"}
            />
          ) : (
            <div
              className={`w-2 h-2 rounded-full ${
                isActive ? activeStyles.bar1 : "bg-primary-500"
              }`}
            />
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={`h-1 w-4 rounded-full ${
              isActive ? activeStyles.bar2 : "bg-primary-300"
            }`}
          />
          <div
            className={`h-1 w-2 rounded-full ${
              isActive ? activeStyles.bar3 : "bg-primary-200"
            }`}
          />
        </div>
      </div>
