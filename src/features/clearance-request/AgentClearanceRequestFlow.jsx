...
export const AgentClearanceRequestFlow = () => {
  ...
  const [step, setStep] = useState(() => {
    ...
    if (selectedRequest?.status) {
      ...
      if (status === "HPG_VERIFIED" || status === "MVC_MEC_VALIDATED") return 3;
      if (status === "CERTIFICATE_ISSUED") return 4;
    }
  ...
  useEffect(() => {
    ...
    if (selectedRequest.currentStep && !hasSyncedStep.current) {
      ...
      else if (selectedRequest.status === "HPG_VERIFIED" || selectedRequest.status === "MVC_MEC_VALIDATED") derivedStep = 3;
      else if (selectedRequest.status === "CERTIFICATE_ISSUED") derivedStep = 4;
  ...
  const canNext = () => {
    ...
    if (step === 3) return Boolean(requestStatus === "CERTIFICATE_ISSUED" || requestStatus === "MVC_MEC_VALIDATED");
    return false;
  };
