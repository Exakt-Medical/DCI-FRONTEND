...
export const OrCrMismatchModal = ({ mismatches, isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = () => {
    ...
    mismatches.forEach((m) => {
      resolvedValues[m.field] = selections[m.field] === "OR" ? m.orValue : m.crValue;
    });
        ...
        <div className="flex items-start gap-3 bg-amber-50 p-4 rounded-lg border border-amber-200">
          ...
          <div>
            ...
            <p className="text-amber-700 text-xs mt-1 leading-relaxed">
              We noticed differences between the data extracted from your OR and CR.
          ...
          {mismatches.map((m) => (
            <div key={m.field} className="border border-gray-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div
                  onClick={() => handleSelect(m.field, "OR")}
                  className={`cursor-pointer rounded-lg border-2 p-3 text-center transition-colors ${
                    selections[m.field] === "OR"
                      ? "border-[#0059b5] bg-[#f8fbff]"
                      : "border-gray-200 hover:border-gray-300"
                  }`}
                >
                  <p className="text-[10px] text-gray-500 uppercase font-bold mb-1">From OR</p>
                  <p className="font-semibold text-gray-900 truncate" title={m.orValue || "N/A"}>
                    {m.orValue || <span className="text-gray-400 italic">Empty</span>}
                  </p>
                </div>
                <div
                  onClick={() => handleSelect(m.field, "CR")}
                  className={`cursor-pointer rounded-lg border-2 p-3 text-center transition-colors ${
                    selections[m.field] === "CR"
                      ? "border-[#0059b5] bg-[#f8fbff]"
                      : "border-gray-200 hover:border-gray-300"
                  }`}
                >
                  <p className="text-[10px] text-gray-500 uppercase font-bold mb-1">From CR</p>
                  <p className="font-semibold text-gray-900 truncate" title={m.crValue || "N/A"}>
                    {m.crValue || <span className="text-gray-400 italic">Empty</span>}
                  </p>
                </div>
              </div>
            </div>
