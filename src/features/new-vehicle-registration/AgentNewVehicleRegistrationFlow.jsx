...
import { AGENT_REG_STEPS, NEW_REG_DOC_TYPES, VEHICLE_TYPE, VEHICLE_TYPE_LABELS, VALIDATION_STATE, emptySalesInvoice, emptyCsr, emptyCtpl, emptyStencil, isAllDocumentsComplete, generateRegRefNumber } from "./utils/newVehicleRegistrationUtils";
...
export const AgentNewVehicleRegistrationFlow = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { error: showError, success: showSuccessAlert } = useAlert();

  const flowSteps = AGENT_REG_STEPS;
  ...
  const [transactionAssigned, setTransactionAssigned] = useState(false);
  ...
  const handleProceedToTransactionCode = () => {
    setStep(2);
  };

  const handleAssignTransactionCode = () => {
    setIsAssigningCode(true);
    setTimeout(() => {
      const code = `TXN-${Math.floor(Math.random() * 90000) + 10000}`;
      setTransactionCode(code);
      setTransactionAssigned(true);
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center pt-4 border-t border-gray-200">
        ...
        <Button
          onClick={handleProceedToTransactionCode}
          disabled={!canProceedStep1}
        >
          Assign Transaction Code <ChevronRight size={16} />
        </Button>
      </div>
    </div>
  );
      ...
      <Card className="p-6">
        <div className="text-center mb-6">
          <div className="w-14 h-14 bg-[#0059b5]/10 rounded-full flex items-center justify-center mx-auto mb-3">
            <Hash size={28} className="text-[#0059b5]" />
          </div>
          <h2 className="text-lg font-bold text-gray-900">Transaction Code</h2>
          <p className="text-sm text-gray-500 mt-1">
            Assign a transaction code from your available credits.
          </p>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-6">
          ...
          <div className="border-t border-gray-200 pt-2 mt-2">
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-gray-900">Available Credits</span>
        ...
        {transactionAssigned ? (
          <div className="text-center py-4">
            ...
            <h3 className="text-base font-bold text-green-700">Transaction Code Assigned</h3>
            <p className="text-sm font-mono font-bold text-[#0059b5] mt-1">{transactionCode}</p>
          </div>
        ) : (
          <div className="space-y-3">
            <Button
              onClick={handleAssignTransactionCode}
              disabled={isAssigningCode || availableCredits <= 0}
              className="w-full"
              size="lg"
            >
              {isAssigningCode ? (
                <><Spinner className="mr-2" /> Assigning Code...</>
              ) : (
                <><Hash size={18} className="mr-2" /> Assign Transaction Code</>
              )}
            </Button>
            {availableCredits <= 0 && (
              <p className="text-xs text-red-500 text-center">
                No available credits. Please purchase transaction credits first.
              </p>
            )}
          </div>
        )}
