...
export const PurchaseHistoryTable = ({ purchaseHistory, formatCurrency, copyToClipboard }) => {
    ...
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold text-gray-900">Purchase History</h3>
        </div>
      ...
      {filteredHistory.length > 0 ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600">Plan</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600">Premium</th>
              ...
              {filteredHistory.map((policy) => (
                <tr key={policy.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <code className="text-xs font-mono font-medium text-gray-900">{policy.policyNumber}</code>
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">{policy.productName}</td>
                  <td className="px-4 py-3 text-sm font-medium text-primary-600">{formatCurrency(policy.premium)}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{policy.expirationDate}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded ${getStatusColor(policy.status)}`}>
                      {policy.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <code className="text-xs font-mono text-gray-500">{policy.voucherCode}</code>
                      <button 
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Purchase your first CTPL policy above</p>
        </div>
      )}
    </Card>
  );
